  ...
  class AccountCenterManager {
    ...
    async createAccountCenter(acConfig) {
      ...
      try {
        ...
        // 授权db
        const sqlGrant = `grant all privileges on \`${acConfig.prefixDatabase}%\`.\`*\` to \`${acConfig.configDBUser}\`@\`%\` identified by '${acConfig.configDBPassword}' `;
        await tr.query(sqlGrant);
    ...
    get tableSchema() {
      return `create table if not exists \`:dbName\`.\`:tableName\` (
            ...
            IDENTITY varchar(92) not null default '',
